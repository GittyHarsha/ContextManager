// plugin/server-src/contextmanager-mcp.ts
...
function resolveProject(projects, selector) {
  ...
  return projects.find(
    (project) => project.id === selector || project.name.toLowerCase() === normalized || (project.rootPaths || []).some((rootPath) => rootPath.toLowerCase() === normalized)
  );
}
...
server.registerTool(
  ...
  async () => {
    ...
    return textResult(
      projects.length ? projects.map((project) => `- ${project.name} (${project.id})${project.rootPaths?.[0] ? ` \u2014 ${project.rootPaths[0]}` : ""}`).join("\n") : "No ContextManager projects found.",
...
${(note.relatedFiles || []).join(" ")}`, query)
    ...
    return textResult(
      results.length ? results.map((result) => `- [${result.kind}] ${result.title} \u2014 ${result.projectName}`).join("\n") : `No knowledge matches found for "${query}".`,
...
server.registerTool(
  ...
  async ({ project, status, limit }) => {
    ...
    const projectRecord = project ? resolveProject(projects, project) : void 0;
    const filtered = sessions.filter((session) => !status || session.status === status).filter((session) => {
      if (!projectRecord) {
        return true;
      }
      return (session.bindingSegments || []).some((segment) => segment.projectId === projectRecord.id && segment.endSequence === void 0);
    }).sort((left, right) => (right.lastActivityAt || 0) - (left.lastActivityAt || 0)).slice(0, limit);
    return textResult(
      filtered.length ? filtered.map((session) => `- ${session.label || session.sessionId} [${session.origin || "unknown"}] \u2014 ${session.status || "pending"}${typeof session.pendingCaptureCount === "number" ? ` (${session.pendingCaptureCount} pending)` : ""}`).join("\n") : "No tracked sessions found.",
...
var REGISTRY_FILE = path.join(os.homedir(), ".contextmanager", "agent-registry.json");
var BUS_FILE = path.join(os.homedir(), ".contextmanager", "agent-bus.jsonl");
var BUS_CURSORS_FILE = path.join(os.homedir(), ".contextmanager", "bus-cursors.json");
...
server.registerTool(
  ...
  async ({ project }) => {
    ...
    const summary = agents.map((a) => {
      const meta = Object.keys(a.meta || {}).length > 0 ? ` | meta: ${JSON.stringify(a.meta)}` : "";
      const age = Math.round((Date.now() - a.lastSeenAt) / 1e3);
  ...
  async ({ project, limit }) => {
    ...
    for (const line of content.split("\n")) {
      ...
      try {
        ...
        if (msg.ttl && now - msg.timestamp > msg.ttl * 1e3) {
          continue;
        }
